...
fn main() -> Result<(), Box<dyn Error>> {
	...
	image_buffer.enumerate_rows_mut().par_bridge().for_each(|(_, row)| {
		for (x, y, pixel) in row {
			let y_coord = ((y as f64) - (HEIGHT as f64 / 2.0)) * SCALE_FACTOR;
			let x_coord = ((x as f64) - (WIDTH as f64 / 2.0)) * SCALE_FACTOR;

			let projected_ray = Ray::new(Vec3::new(0.0, 0.0, 0.0), Vec3::new(x_coord, y_coord, -1.0));
